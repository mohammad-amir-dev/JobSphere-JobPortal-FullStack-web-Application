...
const SignUp = () => {
    ...
    const handleSubmit = () => {
        ...
        if(valid===true){
            registerUser(data).then((res) => {
                ...
                console.log(err);
                setLoading(false);
              const message = err?.response?.data?.errorMessage || err?.message || "Registration failed. Please try again.";
              errorNotification("Registration Failed", message);
        });
  ...
  /> <div   className="flex flex-col justify-center w-1/2 gap-3 px-20 sm-mx:py-20 sm-mx:w-full bs-mx:px-10 md-mx:px-5">
        ...
        <div className="text-center sm-mx:text-sm xs-mx:text-xs">Have an account?  <span className="cursor-pointer text-bright-sun-400 hover:underline sm-mx:text-sm xs-mx:text-xs" onClick={()=>{navigate("/login");setFormError(form) ;setData(form)}}>Login</span> </div>
